...
#    Updated: 2026/01/19 09:43:40 by tpanou-d         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
DIR_SRCS	:= srcs/regular
DIR_OBJS	:= objs/regular
...
$(DIR_OBJS)/%.o: $(DIR_SRCS)/%.c
	@mkdir -p $(dir $@)
	@$(CC) $(CFLAGS) -c $< -o $@
	@curr=$$(find objs/regular -iname *.o | wc -l); \
	total=$$(find srcs/regular -iname *.c | wc -l); \
...
$(DIR_OBJS_BON)/%.o: $(DIR_SRCS_BON)/%.c
	@mkdir -p $(dir $@)
	@$(CC) $(CFLAGS) -c $< -o $@
	@curr=$$(find objs/bonus -iname "*.o" | wc -l); \
	total=$$(find srcs/bonus -iname *.c | wc -l); \
	printf "\r\e[?25lCompiling : \e[1m⟬\e[107m%*s\e[0m%*s\e[1m⟭\e[0m %d/%d" \
		$$curr "" $$(($$total - $$curr)) "" $$curr $$total
